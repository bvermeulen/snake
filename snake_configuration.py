...
class SnakeSetup:
    ...
    def __init__(self):
        ...
        self.myfont = pygame.font.Font(None,22)
        self.radius = int(self.cell_dim_x/2)
        ...
        self.m_w_o = (int(self.a_br[0] - STATUS_X - PADDING - self.m_w_x), \
                      int(self.a_br[1] + status_y - self.m_w_y + PADDING))        

        self.text_x = self.a_w_x - STATUS_X - self.m_w_x - 2 *PADDING
        self.text_y = status_y

        self.s_w_o            = (int(self.a_br[0] - STATUS_X), int(self.a_br[1]))
        self.r_action_window  = (self.a_tl, (self.a_w_x, self.a_w_y))
        self.r_status_window  = (self.s_w_o, (STATUS_X, status_y + PADDING))
        self.r_monitor_window = (self.m_w_o, (self.m_w_x, self.m_w_y))
        self.r_text_window    = ((self.a_bl[0], self.a_bl[1] + PADDING), (self.text_x, self.text_y))
        ...
        self.screen = pygame.display.set_mode((self.main_x, self.main_y))
        self.pause_SMB = pygame.image.load('data/Pause.png').convert_alpha()
        self.run_SMB   = pygame.image.load('data/Triangle.png').convert_alpha()
