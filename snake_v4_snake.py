...
from snake_v4_tools import (BLACK, WHITE, RED, GREY, YELLOW, BLUE, ORANGE, GREEN, LWIDTH, 
                            SnakeSetup, eye, randomvector, hex_color
                           )
...
def mouse_pressed(grid, cell):
    ...
    if grid[0] < setup.cells_x and grid[0] >= 0 and grid[1] < setup.cells_y and grid[1] >= 0:
        ...
        while i <= snake_number and create == True:
            ...
            '''
            if grid[0] == snake[i-1].head[0] and grid[1] == snake[i-1].head[1]:
                delete_snake(i-1, cell)
                create = False
            i += 1
...
def move_randomly(cell, debug_stats):
    ...
    for i in range(snake_number):
        ...
        if not snake[i].move(cell):   # if snake is stuck in the wall delete the snake
            debug_stats[3] += 1
            delete_snake(i, cell)
            break
