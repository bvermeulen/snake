#!/usr/bin/env python
'''  module snake_v4_control has the following class:
     - Control
'''
from tkinter import Button

from snake_v4_tools import BLUE, ORANGE, BWIDTH, SnakeSetup
from snake_v4_snake import mouse_pressed, reset_snakes, snake_selection
...
class Control:
    ...
    def mouse_action(self, event):
        '''  if event is mouse clicked then determine the location through
             cell indices first check in mouse point is not on a button though
        '''
        if self.pause and not self.in_button:
            ...
            if mouse_pressed(_m_pos, self.cell):
                pass
