...
''' 
    ...
    module snake_v4_tools has the following classes and functions
    Class: 
            - SnakeSetup
            - Cell
    Function: 
...
from tkinter import Tk, Canvas, Label, BOTH, YES, Label
...
class SnakeSetup:
    ...
    def __init__(self, set_window = False):
        ...
        self.s_w_o = (self.a_w_o[0] + a_w_x - STATUS_X, self.a_w_o[1] + a_w_y + PADDING)
        ...
        self.b_w_o.append( (v_w_o[0] - 3 * (PADDING + _bwidth), self.s_w_o[1] + _bheight) )
        self.b_w_o.append( (v_w_o[0] - 2 * (PADDING + _bwidth), self.s_w_o[1] + _bheight) )
        self.b_w_o.append( (v_w_o[0] - 1 * (PADDING + _bwidth), self.s_w_o[1] + _bheight) )
...
def display_text(aw, t_elapsed, snakes, select):
    ...
    '''
    s_num = lambda x: str(x) if x > 0 else 'None'
