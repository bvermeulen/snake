#!/usr/bin/env python
'''  module snake_v5_2_control has the following classes:
     - ButtonControl
     - MouseControl
     - KeyControl
     - Control
...
from snake_v5_2_tools import (root, mroot, aw, logger, BLUE, ORANGE, NOCOLOR,
                              BWIDTH, Setup, Tools,)
...
class ButtonControl:
    ...
    def buttons(self):
        ...
        '''
        logger.info(f'==> button called: {setup.b_w_o}')
        ...
        self.monitor_button = Button(root, text='Monitor',
...
class MouseControl:
    ...
    '''
    def __init__(self):
        aw.bind('<Button>', self.clicked)
        aw.bind('<Double-Button>', self.double_click)
        aw.bind('<ButtonRelease>', self.button_released)
        aw.bind('<B1-Motion>', self.moved)
    ...
    def clicked(self, event):
        ...
        aw.after(250, self.action, event)
    ...
    def moved(self, event):
        '''  define action on when mouse is moved in this case just printing
             the coordinates'''
        if self.setup:
            move_wall_vertex(event, self.button_released_flag, self.cell)

    def action(self, event):
        '''  action on click and double click in this case just printing the
             event '''
        if self.button_released_flag:
            if self.setup:
                mouse_action_wall(event, self.double_click_flag, self.cell)
            ...
            elif self.pause and not self.double_click_flag:
...
class KeyControl:
    ...
    '''
    def __init__(self):
        root.bind('<Key>', self.key_action)
        root.protocol('WM_DELETE_WINDOW', self.exit_program)
        mroot.protocol('WM_DELETE_WINDOW', self.exit_program)
