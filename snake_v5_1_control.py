...
class Control:
    '''  class Control
         Following keystrokes are defined:
         ...
         Button actions are equivalent to Space, (s, S), (w, W), (c, C), (m, M)
         and Escape. When the Exit window is pressed program is ended.

         When in wall setup mode following mouse actions are defined in the
         module _walls:
         ...
         -   __repr__
    '''
    def __init__(self, root, aw, mw, cell):
        self.run = True
        self.pause = True
        self.left = False
        self.right = False
...
#       I am no longer sure why a check had to be made for this
        if self.setup:
